...
while IFS=, read mac user password authid number; do
...
#check the P value of Sip User ID in the txt file, and change the value in different conditions.
	...
	sed -e "/^<mac>/s:.*:<mac>$mac</mac>:g" -e "/^<P35>/s:.*:<P35>$user</P35>:g" -e "/^<P270>/s:.*:<P270>$number</P270>:g" -e "/^<P34>/s:.*:<P34>$password</P34>:g" -e "/^<P36>/s:.*:<P36>$authid</P36>:g" -e "/^<P/s:<P:    <P:" -e "s///g" < $1tmp4.xml > "$1cfg$mac.xml"
